    ...
    private boolean scan_it(int x, int y, int z, int dx, int dy, int dz) {
        ...
        for (i = - dy; i <= dy; ++i) {
            for (j = - dz; j <= dz; ++j) {
                ...
                if ((bid = this.worldObj.getBlock(x - dx, y + i, z + j)) != Blocks.tallgrass && bid != Blocks.yellow_flower && bid != Blocks.red_flower && bid != Blocks.deadbush && bid != Blocks.double_plant || (d = dx * dx + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x - dx;
                this.ty = y + i;
                this.tz = z + j;
        ...
        for (i = - dx; i <= dx; ++i) {
            for (j = - dz; j <= dz; ++j) {
                ...
                if ((bid = this.worldObj.getBlock(x + i, y - dy, z + j)) != Blocks.tallgrass && bid != Blocks.yellow_flower && bid != Blocks.red_flower && bid != Blocks.deadbush && bid != Blocks.double_plant || (d = dy * dy + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x + i;
                this.ty = y - dy;
                this.tz = z + j;
        ...
        for (i = - dx; i <= dx; ++i) {
            for (j = - dy; j <= dy; ++j) {
                ...
                if ((bid = this.worldObj.getBlock(x + i, y + j, z - dz)) != Blocks.tallgrass && bid != Blocks.yellow_flower && bid != Blocks.red_flower && bid != Blocks.deadbush && bid != Blocks.double_plant || (d = dz * dz + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x + i;
                this.ty = y + j;
                this.tz = z - dz;
